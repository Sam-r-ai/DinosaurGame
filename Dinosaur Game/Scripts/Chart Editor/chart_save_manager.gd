	...
	for note in note_dictionary:
		...
		if note_data.size() >= 3:
			...
			if new_note.hold_time > 0:
				var new_held_note = chart_editor.note_hold_prefab.instantiate();
				new_held_note.global_position = new_note.global_position;
				note_parent.add_child(new_held_note);
				new_held_note.update_length(chart_editor.seconds_to_distance(new_note.hold_time));
